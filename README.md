...
# Editorial articles and cover illustrations

After deploying this update, open an existing draft and save any unsaved edits. Choose **Write full article** for an AI narrative, or **Refresh evidence-based draft** for a deterministic starter. Both actions leave an unsaved draft for review; they do not overwrite the saved article or publish anything. Headline ideas are optional. The page writer targets a developed article with sections rather than a short statistics summary.

Numbers in AI narratives are expanded from allowlisted, complete factual sentences. Code calculates shares and supported comparisons; an unknown fact token or literal numeric claim rejects the response without changing the saved draft. This is a numeric guardrail, not a guarantee of factual or editorial correctness: review the surrounding language. No new research runs during writing. Year-over-year statements require an existing comparison snapshot. Missing comparison rows are never treated as zero.

The **Verified statistics** and **Methods, definitions and limitations** appendix is rebuilt from the saved evidence on save/export. Crashes and people are labeled separately. Missing-intersection exclusions are explained only for intersection groupings. Road-name warnings flag likely spacing/spelling variants among returned results; they never merge labels, recompute totals, or claim to identify every alias. Existing narrative text is preserved until you explicitly rewrite it; older prose may still need correction.

The cover panel includes a ready-to-use truck illustration. Selecting/removing a cover makes the article unapproved until you save and approve it again. **Generate new cover · API usage** queues a worker job using the same server-side `OPENAI_API_KEY`; the worker defaults to `OPENAI_IMAGE_MODEL=gpt-image-2` (optional override). Image-model access and billing must be enabled on your OpenAI account. See the [official image generation guide](https://developers.openai.com/api/docs/guides/image-generation). No raw crash rows, working article text, or identifying data is sent for cover generation—only a fixed, cohort-specific illustration brief.

Generated covers are kept in the existing private Supabase bucket under `editorial-covers/`, separate from the immutable CSV originals. A completed image waits for explicit selection; it never changes draft text, attaches itself, or publishes automatically. Failed image jobs require an explicit retry to avoid repeatedly billing API calls. A retry reuses a successfully archived image; an interrupted API request before storage may incur another charge when retried. Review every image for visual artifacts and appropriateness. The disclosure **“AI-generated illustration. Not a photograph of an actual crash or a verified location.”** is retained in exported pages.

Export **Complete article + image package** for HTML, cover, deterministic SVG chart, aggregate CSV, evidence JSON, article text, and separately labeled newsletter/social adaptations. Adaptations are evidence-based starters requiring review before sending/posting. Keep the image alongside `index.html`; publication ZIPs use the article's own folder and canonical URL. Standalone HTML embeds its selected cover for portability. No live DNS, website, email or social changes are made by export. Packages are capped at 150 MB.
...
Unsaved/unapproved work can be previewed with a review-copy label. Save, review and approve the article to unlock **Download PNG** and **Download SVG**. Both downloads fetch the approved saved version from the authenticated export endpoint; PNG conversion happens in the browser, without uploading the graphic elsewhere. If PNG conversion fails, the scalable SVG remains available. The article ZIP (and each article in a publication ZIP) includes `infographic.svg` and `infographic-methodology.txt`, alongside full evidence and CSV. PNG is a separate browser download, not included in ZIPs.

Infographics show current-period counts only, not a trend inferred from incomplete comparisons. Counts describe crash events, not people or exposure-adjusted risk. Vehicle groups can overlap. Road-name variants are not merged. Old pre-AM/PM hour snapshots are blocked. Long displayed headlines/labels may be abbreviated; complete labels and methodology are preserved in SVG accessibility text and the evidence package. No new environment variables or database migrations are required.
